...
class _TransactionsScreenState extends State<TransactionsScreen> with SingleTickerProviderStateMixin {
  ...
  DateTime? _parseDateKey(String dateKey) {
    ...
    try {
      return DateFormat('dd MMM yyyy').parse(dateKey);
    } catch (e) {
      print('Error parsing dateKey "$dateKey": $e');
      return null; // Should not happen if keys are generated by _formatDate
    }
  }
  ...
  (int, int) _computeDownloadDateRange() {
    ...
    // Default to today's date range if no filters are set
  ...
  String _getDownloadRangeText() {
    ...
    if (selectedTab == 'POS') {
      ...
      if (qrDateRangeType == 'choose_date' && qrFilterStartDate != null && qrFilterEndDate != null) {
        return '${qrFilterStartDate!.day}/${qrFilterStartDate!.month}/${qrFilterStartDate!.year} - ${qrFilterEndDate!.day}/${qrFilterEndDate!.month}/${qrFilterEndDate!.year}';
      } else if (qrDateRangeType == 'this_month') {
        return 'This Month';
      } else if (qrDateRangeType == 'last_30_days') {
        return 'Last 30 Days';
...
// Add this method to handle the actual download
  Future<void> _downloadTransactions() async {
    ...
    print('Download date range: ${fromDate} to ${toDate}');
  ...
  Future<void> _downloadStaticQRTransactions(int fromDate, int toDate) async {
    try {
      print('Downloading Static QR transactions: fromDate=$fromDate (${DateTime.fromMillisecondsSinceEpoch(fromDate * 1000)}), toDate=$toDate (${DateTime.fromMillisecondsSinceEpoch(toDate * 1000)})');
      ...
      print('Static QR Download URL: ${uri.toString()}');
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to download Static QR transactions. Status: ${response.statusCode}')),
        );
      }
    } catch (e) {
      print('Error downloading Static QR transactions: $e');
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildPOSTransactions() {
    ...
    for (var transaction in transactions) {
      try {
        ...
        // Ensure "Today" is used as the key for grouping if it's today's date
        ...
        print('Error parsing date for grouping: ${transaction['time']} - $e');
      }
    }
    ...
    dateKeys.sort((a, b) {
      ...
      if (dateA == null) return 1; // Put nulls (errors) at the end
      if (dateB == null) return -1; // Put nulls (errors) at the end
  ...
  Widget _buildStaticQRTransactions() {
    ...
    for (var transaction in transactions) {
      try {
        final transactionDate = _parseDateTime(transaction['time']);
        String dateKey = _formatDate(transactionDate);
        if (dateKey == _todayMarker) { // Use "Today" for the header text
          dateKey = "Today";
        }
        if (!groupedTransactions.containsKey(dateKey)) {
          groupedTransactions[dateKey] = [];
        ...
        print('Error parsing date for grouping in Static QR: ${transaction['time']} - $e');
      }
    }
              ...
              MaterialPageRoute(
                builder: (context) => StaticQRChargeSlip(
                  transactionData: {
                    ...
                    "customerVpa": transaction['cardNumber'],
                    "creditVpa": selectedVPA ?? 'N/A',
                  },
                ),
              ),
            );
        ...
        if (responseData['status'] == 'OK') {
          ...
          for (var transaction in content) {
            newTransactions.add({
              ...
              // Add any additional fields needed for the charge slip
              "merchantTransactionId": transaction['merchantTransactionId'] ?? '',
              "creditVpa": transaction['creditVpa'] ?? '',
            });
          }
